...
export const createProject = async (req, res) => {
    try {
        ...
        if (!workspace.members.some((member) => member.userId === userId && member.role === "ADMIN")) {
            return res.status(403).json({ message: "You don't have persmission to create projects in this workspace" })
        }
    ...
    try {
        ...
        if (!workspace.members.some((member) => member.userId === userId && member.role === "ADMIN")) {
            ...
            if (!project) {
                ...
                return res.status(403).json({ message: "You don't have persmission to update projects in this workspace" })
            }
        }
